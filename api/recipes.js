...
async function getRecipes() {
  const res = await r2Request('GET', 'recipes.json', '');
  if (res.status === 404 || res.status === 403) return [];
  if (res.status !== 200) throw new Error('R2 GET failed: ' + res.status + ' ' + res.body.slice(0, 200));
  return JSON.parse(res.body);
}

async function putRecipes(recipes) {
  const body = JSON.stringify(recipes);
  const res = await r2Request('PUT', 'recipes.json', body);
  if (res.status !== 200) throw new Error('R2 PUT failed: ' + res.status + ' ' + res.body.slice(0, 200));
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'DELETE') {
      ...
      const recipes = await getRecipes();
      await putRecipes(recipes.filter(r => r.id !== id));
      return res.status(200).json({ ok: true });
    }
